---
title: "Partie visualisation"
output: html_notebook
---

```{r}
# PACKAGES

library(corrplot)
```

# 1. Sans corrélation entre les variables explicatives

## 1. Génération des données

```{r}
set.seed(123)
n <-  100

# Résidu 
sigma2 <- 1
bruit <- rnorm(n, mean = 0, sd = sqrt(sigma2))

# Vecteur
X1 <- runif(n, min = -2, max = 2)
X2 <- runif(n, min = -2, max = 2)

# Coefficient beta 
beta <-  c(1, 1, 1)

# Réponse Y
Y <- beta[1] + beta[2]*X1 + beta[3]*X2 + bruit

# RL
reg <- lm(Y ~ X1 + X2)
```

## 2. Graphique des corrélation

```{r}
#visualisation de la corrélation entre les variables 
par(mfrow = c(1, 3))
plot(Y, X1, main = "x1 ~ Y")
plot(Y, X2, main = "x2 ~ Y")
plot(X1, X2, main = "x1 ~ x2")
```
On voit bien que la corrélation n'existe pas entre les 2 variables explicatives 

```{r}
#Matrice de corrélation et plot 
X = cbind(X1,X2)
cor_mat = cor(X)
corrplot(cor_mat, type="upper", order="hclust", tl.col="black", tl.srt=45)

```
Ceci confirme ce qu'on avait déduit du nuage de points.

## 3. Histogramme des résidus 

```{r}


hist(reg$residuals, col = "skyblue", main = "Distribution des résidus")

```
Les résidus suivent bien une loi normale centrée.

## 4. Graphes des résidus

```{r}
# Graphe des résidus en fonction de Y
plot(Y, reg$residuals)
abline(h = 0, col = "red")

# Graphe des résidus en fonction de X1
plot(X1, reg$residuals)
abline(h = 0, col = "red")

# Graphe des résidus en fonction de X2
plot(X2, reg$residuals)
abline(h = 0, col = "red")
```
Le graphique des résidus en fonction de Y n'est pas celui attendu, on voit une légère tendance, malgré le fait qu'il n'y a auncune corrélation en X1 et X2.


```{r}
#Matrice de corrélation et plot 
X = cbind(Y, reg$residuals)
cor_mat = cor(X)
corrplot(cor_mat, type="upper", order="hclust", tl.col="black", tl.srt=45)

```
```{r}
plot(reg$fitted.values, reg$residuals)
abline(h = 0, col = "red")


```

## 5. QQplot

```{r}
# QQ-plot des résidus
qqnorm(residuals(reg), main = "QQ-plot des résidus")
qqline(residuals(reg), col = "red", lwd = 2)
```
Les résidus sont assez bien alignés avec la droite diagonale, donc cela confirme que les résidus suivent une loi normale.

# 2. Avec corrélation entre les variables explicatives

## 1. Génération des données

```{r}
set.seed(123)
n <-  100

# Résidu 
sigma2 <- 1
bruit <- rnorm(n, mean = 0, sd = sqrt(sigma2))

# Vecteur
X1 <- runif(n, min = -2, max = 2)

# Génération de X2 corrélé à X1
rho <- 0.8  # corrélation souhaitée (entre -1 et 1)
X2 <- rho * X1 + sqrt(1 - rho^2) * runif(n, min = -2, max = 2)

# Coefficient beta 
beta <-  c(1, 1, 1)

# Réponse Y
Y <- beta[1] + beta[2]*X1 + beta[3]*X2 + bruit

# RL
reg <- lm(Y ~ X1 + X2)
```

## 2. Graphique des corrélation

```{r}
#visualisation de la corrélation entre les variables 
par(mfrow = c(1, 3))
plot(Y, X1, main = "x1 ~ Y")
plot(Y, X2, main = "x2 ~ Y")
plot(X1, X2, main = "x1 ~ x2")
```

```{r}
#matrice de corrélation et plot 
X = cbind(X1,X2)
cor_mat = cor(X)
corrplot(cor_mat, type="upper", order="hclust", tl.col="black", tl.srt=45)
```

## 3. Histogramme des résidus 

```{r}
hist(reg$residuals, col = "skyblue", main = "Distribution des résidus")
```

## 4. Graphes des résidus

```{r}
# Graphe des résidus en fonction de Y
plot(Y, reg$residuals)

# Graphe des résidus en fonction de X1
plot(X1, reg$residuals)

# Graphe des résidus en fonction de X2
plot(X2, reg$residuals)
```

## 5. QQplot

```{r}
# QQ-plot des résidus
qqnorm(residuals(reg), main = "QQ-plot des résidus")
qqline(residuals(reg), col = "red", lwd = 2)
```

Donc, malgré la liaison, on ne voit pas de réelle différence entre les graphiques. Ils suffisent donc pas à l'analyse.

# 3. Avec plus de variables explicatitves

## 1. Génération des données

```{r}
generate.lm <- function(n.train, p, p0, sigma2, n.test=10*n.train) {
  ## génération ensemble test/apprentissage
  ## Rq : Le test peut être grand ici, car il n'impacte pas la taille de l'échantillon d'apprentissage
  ## (Plus le test sera grand, plus les modèles seront évalués avec précision.)
  
  n <- n.train + n.test
  train <- 1:n.train
  test <- (n.train+1):n
  
  ## vecteur des vrais coefficients
  beta <- numeric(p)
  S.star <- sample(1:p, p0) ## vrai support S*
  beta[S.star] <- runif(p0, 1, 2)*sample(c(-1,1),p0, replace=T)
  ## bruit blanc gaussien de variance sigma2
  noise <- rnorm(n, sd=sigma2)
  ## prédicteurs sans structure particulière
  X <- matrix(rnorm(n*p), n, p)
  ## vecteur d'observation des réponses
  y <- ## vecteur des vrais coefficients
    beta <- numeric(p)
  S.star <- sample(1:p, p0) ## vrai support S*
  beta[S.star] <- runif(p0, 1, 2)*sample(c(-1,1),p0, replace=T)
  ## bruit blanc gaussien de variance sigma2
  noise <- rnorm(n, sd=sigma2)
  ## prédicteurs sans structure particulière
  X <- matrix(rnorm(n*p), n, p)
  ## vecteur d'observation des réponses
  y <-  X%*%beta + noise
  
  list(y=y, X=X, beta=beta, sigma2=sigma2, train=train, test=test)
}

# data generation
dataset<-generate.lm(n.train=100,p=50,p0=10, sigma2=1,n.test=100)
support<-(dataset$beta!=0)*1 # support[j]=1 si beta_j est non nul
fit<-lm(dataset$y~dataset$X) 
```

## 2. Graphique des corrélation

```{r}
#matrice de corrélation et plot 
X = dataset$X
cor_mat = cor(X)
corrplot(cor_mat, type="upper", order="hclust", tl.col="black", tl.srt=45)
```
## 3. Histogramme des résidus 

```{r}
hist(fit$residuals, col = "skyblue", main = "Distribution des résidus")
```

## 4. Graphes des résidus

```{r}
# Graphe des résidus en fonction de Y
plot(dataset$y, fit$residuals)
```

## 5. QQplot

```{r}
# QQ-plot des résidus
qqnorm(residuals(fit), main = "QQ-plot des résidus")
qqline(residuals(fit), col = "red", lwd = 2)
```